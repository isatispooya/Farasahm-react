...
const Menu = (props) => {
  ...
  const toggleActMenu = (index) => {
    ...
    if (updatedMenu[index].sub.length == 0) {
      ...
      setMenuTop((prevMenu) =>
        prevMenu.filter((menuItem) => menuItem !== updatedMenu[index].url)
      );
    } else {
                  ...
                  {item["sub"].length > 0 ? (
                    <>
                      <div key={index} className="itemMenu">
                        ...
                        <h3 onClick={() => toggleActMenu(index)}>
                          {item["name"]}
                        </h3>
                      </div>
                    ...
                    <div className="itemMenu">
                      <p>{menuFullList[item["url"]]}</p>
                      <h3 key={index} onClick={() => toggleActMenu(index)}>
                        {item["name"]}
                      </h3>
                    </div>
                  )}
