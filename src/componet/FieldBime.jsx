...
const FieldBime = ({ config, setConfig }) => {
  ...
  const Remove = (id) => {
    let i_field_list = config.insurance.insurance_field;
    i_field_list = i_field_list.filter((i) => i !== id);
    let insurance = { ...config.insurance, insurance_field: i_field_list };
    setConfig({ ...config, insurance: insurance });
  };
  const search_field_filter = (e) => {
              ...
              {config.insurance.insurance_field &&
                config.insurance.insurance_field.length > 0 && (
                  <div className="flex flex-wrap gap-4 mt-4">
                    {config.insurance.name.map((insurance_field) => (
                      <div
                        key={insurance_field}
