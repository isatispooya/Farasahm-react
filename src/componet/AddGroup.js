...
const AddGroup = (props) =>{
    ...
    if (props.codesDf.length>0 && props.addGroupShow) {
        var tablesub = new Tabulator("#data-table-sub", {
            ...
            index:"کد",
            columns:[
                {title:"کد", field:"کد", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:4,headerFilter:"input"},
                {title:"نام", field:"نام", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:4,headerFilter:"input"},
                {title:"نام خانوادگی", field:"نام خانوادگی", hozAlign:'center',headerHozAlign:'center',resizable:true, widthGrow:4,headerFilter:"input"},
    ...
    const setGrouping = () =>{
        ...
        }else if(rowSelected.length==0){alert('هیچ ردیفی انتخاب نشده است')
        }else{
            axios({method:'POST', url:OnRun+'/setgrouping',data:{access:props.access,name:nameGroup,members:rowSelected}
            }).then(response=>{
