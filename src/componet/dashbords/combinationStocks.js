...
const CombineStocks = () => {
  ...
  const get = () => {
    axios
      .post(OnRun + "/getassetmixoraqsah", { access: access })
      .then((response) => {
  ...
  const Options = {
    ...
    plugins: {
      legend: {
        display: true,
        title: { display: false, text: "ترکیب اوراق سهام" },
  ...
  return (
    <div className="py-5">
      <h1 className="">ترکیب اوراق سهام</h1>
      <div className="flex justify-between mx-10 items-center">
        <div className="my-5">
          <div className="flex flex-col bg-rose-300 ring-1 ring-rose-300 py-3 items-center my-5 px-10  rounded-lg bg-opacity-35 ">
            <h4 className="font-bold text-xl">بازدهی</h4>
            <p className="text-base font-medium">
              {rate === null ? <MiniLoader /> : rate}%
            </p>
          </div>
          <div className="flex flex-col bg-rose-300 ring-1 ring-rose-300 py-3 items-center  px-10  rounded-lg bg-opacity-35 ">
