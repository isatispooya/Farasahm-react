...
const Home = () => {
  ...
  const getCaptcha = () => {
    axios({ method: "POST", url: OnRun + "/captcha" }).then((response) => {
  ...
  const applyPhone = () => {
    if (inputPhone.phone.length != 11) {
      ...
      axios({
        method: "POST",
        url: OnRun + "/applyphone",
        data: { inputPhone: inputPhone, captchaCode: CaptchaCode },
      }).then((response) => {
    ...
    <div className="homePage">
      <div className="login">
        {phase ? (
          <>
            <input
              value={inputPhone.code}
              onChange={(e) =>
                setInputPhone({ ...inputPhone, code: e.target.value })
              }
              placeholder="کد تایید"
              type="phone"
            />
            <button className="ent" onClick={applyCode}>
              تایید
            </button>
            <button className="btnEdt" onClick={() => setPhase(false)}>
              ویرایش
            </button>
          </>
        ) : (
          <>
            <input
              value={inputPhone.phone}
              onChange={(e) =>
                setInputPhone({ ...inputPhone, phone: e.target.value })
              }
              placeholder="شماره همراه"
              type="phone"
            />
            <input
              value={inputPhone.captcha}
              onChange={(e) =>
                setInputPhone({ ...inputPhone, captcha: e.target.value })
              }
              placeholder="کد تصویر"
              type="text"
            />
            <div className="captcha">
              {CaptchaImg == null ? null : (
                <img
                  onClick={getCaptcha}
            ...
            <button className="ent" onClick={applyPhone}>
              تایید
            </button>
          </>
        )}
        {errMsg.length == 0 ? null : (
          <p onClick={() => setErrMsg("")} className="errMsg">
            {errMsg}
          </p>
        )}
      </div>
